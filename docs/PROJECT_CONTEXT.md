...
2. Task 014 两仓实现、Secret 配置和合成真实联调已完成；下一步在 Cloudflare 控制台创建 DeepSeek AI Gateway 网关，再设置受限的 `DEEPSEEK_BASE_URL` 并重跑合成联调。继续保持基础课程离线可用，不切换为用户输入 API Key。
