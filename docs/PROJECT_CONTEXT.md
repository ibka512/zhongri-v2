...
当前 Task 已完成 canonical corpus Schema、固定 9,828/5,906/3,922 目标、真实资产导入、fail-closed 完整性验证、脱敏 source snapshot contract、只读浏览器 source adapter、source-aware staging 和确定性 canonical idMap 应用层契约；下一步是取得可审计 backup fixture 并开始逐域迁移，不伪造用户事实，不修改 FSRS 参数或接入 AI。
