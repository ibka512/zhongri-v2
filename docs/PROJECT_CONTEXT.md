...
当前 Task 已完成 canonical corpus Schema、固定 9,828/5,906/3,922 目标、真实资产导入、fail-closed 完整性验证、脱敏 source snapshot contract、只读浏览器 source adapter、source-aware staging、确定性 canonical idMap、统一 disposition/quarantine 报告、只读 Legacy Source Reader、显式设备来源选择与 IDB/localStorage 分歧报告、Word/Override/Folder/Favorite/Mastery/StudyRecord/FSRS isolated 纵向转换和统一 staging orchestration；下一步是取得可审计 backup fixture 复核真实字段覆盖并开始剩余域迁移，不伪造用户事实，不修改 FSRS 参数或接入 AI。
