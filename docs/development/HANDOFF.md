...
- 当前交接分支：`main`
- 稳定基线提交：`eb406d0`（PR #31 合并后的 main；source adapter 和 source-aware staging 已进入稳定基线）
- 当前实现提交：`eb406d0`（PR #31；只读 v1 source adapter、Port → snapshot 编排和 source-aware staging）
...
- 真实 backup fixture、逐域迁移、quarantine、V01–V25 和激活/回滚仍未完成。
- 首次设置、内容中心、设置与数据安全页。
...
2. 在 Task 015 内实现逐域 transformer、quarantine、V01–V25 验证和激活/回滚。
3. 迁移 Task 通过后再继续首次设置、内容和双语基础切片；Phase 1 验收完成后才进入 AI Gateway（Issue #20）。
