...
- 当前状态：完整 9,828 条 canonical corpus 已从固定 `jp-study` 提交导入，fail-closed 完整性门禁与全量测试已通过，脱敏 source snapshot contract、只读浏览器 source adapter、Port → snapshot 编排、source-aware staging 和确定性 canonical/user idMap 契约已完成；[Issue #23](https://github.com/ibka512/zhongri-v2/issues/23) 仍开放，真实脱敏 fixture 与逐域迁移仍待完成
...
- 真实 backup fixture、逐域 transformer 接线、完整 quarantine/rawArchive、V01–V25 和激活/回滚仍未完成。
- 首次设置、内容中心、设置与数据安全页。
...
2. 将冻结的 idMap 接入 Word/Override/Folder/Favorite/Mastery/StudyRecord/FSRS 等逐域 transformer，并建立 quarantine/rawArchive 报告。
