...
- **Task 015：v1 迁移逐域转换与 canonical 身份层** 正在进行（[Issue #23](https://github.com/ibka512/zhongri-v2/issues/23)）。canonical corpus 已固定并导入 9,828 条，source snapshot contract、只读浏览器 source adapter、Port → snapshot 编排和 source-aware staging 接线已完成；接下来取得真实脱敏 fixture，再实现逐域转换、quarantine、V01–V25 和激活回滚，真实输入到位前不宣称迁移完成。
...
| Task              | 状态   | 说明                                                                                                                                                  |
| ----------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| Task 015 迁移转换 | 进行中 | canonical corpus、snapshot contract、只读 source adapter 与 source-aware staging 已完成；继续真实 fixture、逐域转换、quarantine、V01–V25 和激活回滚。 |
