...
- **Task 009：v1 备份迁移预检** 正在实施。
- Issue：[#10](https://github.com/ibka512/zhongri-v2/issues/10)。
- 当前切片包含 v5+/v4 备份识别、来源摘要、逐域分类、阻断与复核报告、安全 JSON 导出。
- 当前切片严格只读，不包含 IndexedDB 写入、原子迁移、回滚、FSRS 激活、云同步或 AI。

## 待完成

| Task          | 状态   | 说明                                                   |
| ------------- | ------ | ------------------------------------------------------ |
| Task 010 以后 | 未排期 | 只在前一 Task 验收后逐项确认，不从路线图直接推断实现。 |
