...
- **Task 018：内容中心首个只读切片** 已接受并开始实施（见 [TASK-018](./tasks/TASK-018-content-center.md) 和 [ADR-040](./decisions/ADR-040-content-center-readonly-slice.md)）。本切片只复用 canonical repository 展示当前语言的内容摘要与可搜索词条。
...
| Task 018 内容中心   | 实施中               | 只读展示当前语言 canonical 内容摘要与词条搜索；不实现用户词、编辑、收藏、导入或音频。                                                                                                                                                                                                                                                                                                               |
