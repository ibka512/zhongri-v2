...
- **Task 007：GitHub Pages 开发预览部署** 正在实施。
- Issue：[#6](https://github.com/ibka512/zhongri-v2/issues/6)。
- 当前切片包含 Pages 子路径、Hash 路由、PWA 路径、产物校验和自动部署。
- 当前切片不包含后端、账号、同步、自定义域名、公共 Beta 或 AI。

## 待完成

| Task          | 状态   | 说明                                                   |
| ------------- | ------ | ------------------------------------------------------ |
| Task 008 以后 | 未排期 | 只在前一 Task 验收后逐项确认，不从路线图直接推断实现。 |
