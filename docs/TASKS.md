...
- **Task 014：DeepSeek AI Gateway 与结构化任务协议** 已完成 `zhongri-v2` 本地底座实现（见 [TASK-014](./tasks/TASK-014-ai-gateway.md)、[ADR-044](./decisions/ADR-044-ai-gateway-boundary.md) 和 [Issue #20](https://github.com/ibka512/zhongri-v2/issues/20)）。独立 Gateway 仓库、Cloudflare Secret 和供应商联调仍待单独授权。
...
| Task 014 AI Gateway | 本地实现完成，待 Gateway 授权 | `zhongri-v2` 已完成协议 Schema、JSON Schema 导出、`AIGateway` Port、HTTP adapter、运行时 URL 配置、fixture 和 contract tests；独立 Worker、Secret、真实供应商联调和跨仓库发布仍未开始。                                                                                                                                                                                                         |
| Phase 1 后续 Task   | 双语门已通过                  | Task 021 双语闭环已验收；迁移生产级备份/恢复边界仍按现有文档记录，AI Gateway 进入独立 Task 014。                                                                                                                                                                                                                                                                                                |

远端 Issue #20 当前名为 Task 014（AI Gateway）；本地 PWA 底座已完成，独立 Worker/Secret/真实联调仍需单独授权。治理工作不占用产品 Task 编号。
